...
public class AdminLoginController {
    ...
    public ResponseEntity<String> newRegistration(@RequestParam String restaurantId, @RequestParam @Valid String restaurantName,
                                                  @RequestParam @Valid String restaurantAddress, @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.TIME) LocalTime open,
                                                  @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.TIME) LocalTime close, @RequestParam @Valid String closedDay,
                                                  @RequestParam(required = false) MultipartFile restaurantPhoto) throws SessionIdExpiredException, IOException {
        return ResponseEntity.of(Optional.of(restaurantInterface.register(restaurantId, restaurantName, restaurantAddress, open, close, closedDay, restaurantPhoto)));
    }
