...
public class SystemService implements SystemInterface {
    ...
    @Override
    public String cancelOrder(MyOrders myOrders) {
        jdbcTemplate.update("update myOrders set orderStatus = ? where orderId = ?", OrderStatus.CANCELLED.toString(),
                myOrders.getOrderId());
        jdbcTemplate.update("update restaurantOrders set orderStatus = ? where orderId = ?",OrderStatus.CANCELLED.toString(),
                myOrders.getOrderId());
        return "Order cancelled successfully and Payment will be refunded in 1 working day";
    }
